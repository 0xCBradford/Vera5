...
| Behavior | Detail |
|----------|--------|
| **Trigger** | HTTP 429 from AbuseIPDB, OTX, URLScan.io, GreyNoise, or Shodan during a live fetch |
...
**Live enrichment today:** **AbuseIPDB**, **OTX**, **URLScan.io**, **GreyNoise (community)**, and **Shodan** perform HTTPS API calls from the extension when enabled with your keys. Other rows apply to pivot navigation, saved keys that are not sent until live connectors ship, and organizational review before you enable future integrations.
