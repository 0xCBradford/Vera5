...
| Notebook fragments | Typed investigation notebook fragments (`observation` / `tag` / `conclusion` / `hypothesis`) in `chrome.storage.local` under `notebookFragments` via `extension/src/lib/notebookFragmentStorage.ts` (store `schemaVersion` v4, `updatedAt`, `fragments[]`, `iocAttachments` map of IOC key → fragment ids, `sessionAttachments` map of investigation session id → fragment ids, `pageAttachments` map of page scope key → fragment ids, max 512); IOC keys are `type:normalizedValue` (e.g. `ipv4:8.8.8.8`); session attach uses investigation session ids (`vera5-inv-…`); page scope keys are http(s) origin plus optional path prefix (e.g. `https://portal.example.com` or `https://portal.example.com/cases`); workspace snapshot JSON export includes a `notebookFragments` block mapped from this store (plus the `workspaceSnapshotState` mirror) via `extension/src/lib/workspaceSnapshot.ts`; investigation session markdown export appends a **Session notebook** section for session-attached fragments via `extension/src/lib/investigationSessionExport.ts` (popup **IOC export only** omits that section); tray/template exports can optionally append an Obsidian-friendly notebook fragments appendix (`exportTemplates.ts` hook); local-only—no cloud notebook sync |
...
| Relationship edges | Entity-level co-seen / resolved-from relationship edges in `chrome.storage.local` under `relationshipEdges` via `extension/src/lib/relationshipEdgeStorage.ts` (store `schemaVersion`, `updatedAt`, `edges[]`, `minCoOccurrenceCount`, `knownGoodPolicy`, `retentionDays`, max **4096** stored edges after co-occurrence sort); retention prune drops edges older than the configured day window (default 90) on read; hover/tray **Previously appeared with** surfaces at most **64** related entities per focus IOC (clamped 1–256 when overridden); `migrateRelationshipEdgesStore()` upgrades unversioned legacy envelopes and is applied on read; upsert merges by entity-pair identity; clear-all preserves min co-occurrence, known-good policy, and retention days and **never** writes `investigationSessions` (no combined wipe on this path); Options **Relationship memory** edits retention days and **Clear all relationship memory** (browser confirm; sessions stay); local-only—no cloud relationship sync |
