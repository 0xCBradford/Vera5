...
| Relationship edges | Entity-level co-seen / resolved-from relationship edges in `chrome.storage.local` under `relationshipEdges` via `extension/src/lib/relationshipEdgeStorage.ts` (store `schemaVersion`, `updatedAt`, `edges[]`, `minCoOccurrenceCount`, `knownGoodPolicy`, max 4096); `migrateRelationshipEdgesStore()` upgrades unversioned legacy envelopes and is applied on read; upsert merges by entity-pair identity; clear-all preserves min co-occurrence and known-good policy; local-only—no cloud relationship sync |
