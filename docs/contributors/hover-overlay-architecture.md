...
- Shows type, value, enrichment rows, Live/Cached/Error badges, raw JSON panel, copy, recommended next pivots with source-attributed links, composite risk score, reasoning chain when data allows, a local **Notebook** panel (`hoverCardNotebook.ts` + `notebookFragmentStorage.ts`) that lists fragments for the current indicator with **Indicator** / **Session** / **Page** scope tabs and supports inline add/edit/delete for the active scope, and a local **Analyst notes** textarea keyed per indicator (`extension/src/lib/analystNotesSession.ts` with persistence in `extension/src/lib/analystNotesStorage.ts`).
...
- When a local known-good entry matches the indicator, shows a **Known benign** / **Known internal** (or custom label text) badge on the header and a **Matched known-good entry** provenance section (`knownGood.ts` match helpers + `listStoredKnownGoodEntries`) with category, match type, pattern, and entry id. Informational label only—does not override composite risk.
...
- Mirrors the overlay known-good badge (**Known benign** / **Known internal** or custom label text) and matched-entry provenance (category · match type · pattern + entry id) when a stored known-good entry matches.
