...
- Indicators that match local noise rules remain in detection scan results by default. Options **Noise rules → Hide suppressed indicators from scan** (`hideSuppressedFromScan`, default **off**) omits those matches from page/selection scans and highlights when enabled.
