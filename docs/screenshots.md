...
Optional follow-ups (not required for the current placeholder set): context menu **Enrich with Vera5**, **Appeared alongside** on overlay and tray, **Previously appeared with** / **Appeared with** on overlay and tray when local relationship edges exist (list only—not a graph; expect empty on a fresh profile until edges exist), Options **Relationship memory** (retention window, **Clear all relationship memory**—edges only, sessions stay), tray **Suppressed** section after a noise-rule match (or Options **Noise rules** list with import/export controls), Options **Known-good lists** (category toggles, **Export list JSON**, skip-enrich toggle; known-benign badge on overlay/tray when entries match), overlay **Notebook** (typed fragments on Indicator / Session / Page scopes) and workspace **Notebook fragments** / **Export session** with **Session notebook** markdown, **Session export** download menu, **Investigation replay** step-through and **Copy transcript**, **Promote session to collection…**, **Settings Backup** with settings pack and portable **threat profile** export/import (built-in apply buttons, pre-import warning dialog, active-profile indicator), dark-mode variants of captures 1–2.
