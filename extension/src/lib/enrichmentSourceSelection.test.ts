...
describe("enrichment source selection", () => {
  it("treats only explicit true as enabled", () => {
    ...
    expect(isEnrichmentSourceEnabled({ abuseipdb: true, otx: false }, ENRICHMENT_SOURCE.OTX)).toBe(
      false
    );
  ...
  it("limits AbuseIPDB live enrichment to IPv4", () => {
    expect(liveEnrichmentSupportsIocType(ENRICHMENT_SOURCE.ABUSEIPDB, IOC_TYPE.IPV4)).toBe(true);
    expect(liveEnrichmentSupportsIocType(ENRICHMENT_SOURCE.ABUSEIPDB, IOC_TYPE.DOMAIN)).toBe(false);
    expect(liveEnrichmentSupportsIocType(ENRICHMENT_SOURCE.OTX, IOC_TYPE.DOMAIN)).toBe(true);
  });

  it("lists all enabled live sources for an IOC type", () => {
    expect(
      listEnabledLiveEnrichmentSourceIds({ abuseipdb: true, otx: true }, IOC_TYPE.IPV4)
    ).toEqual([ENRICHMENT_SOURCE.ABUSEIPDB, ENRICHMENT_SOURCE.OTX]);
  });

  it("includes VirusTotal and Censys for supported live enrichment", () => {
    expect(
      listEnabledLiveEnrichmentSourceIds(
    ...
    expect(listEnabledLiveEnrichmentSourceIds({ censys: true }, IOC_TYPE.DOMAIN)).toEqual([]);
    ...
    expect(liveEnrichmentSupportsIocType(ENRICHMENT_SOURCE.VIRUSTOTAL, IOC_TYPE.IPV4)).toBe(true);
  });

  it("includes Shodan when enabled for supported indicator types", () => {
    expect(
      listEnabledLiveEnrichmentSourceIds({ shodan: true, abuseipdb: false }, IOC_TYPE.IPV4)
    ).toEqual([ENRICHMENT_SOURCE.SHODAN]);
    expect(listEnabledLiveEnrichmentSourceIds({ shodan: true }, IOC_TYPE.URL)).toEqual([]);
  });

  it("prefers AbuseIPDB when both live sources are enabled for IPv4", () => {
    expect(
      resolveEnabledLiveEnrichmentSourceId({ abuseipdb: true, otx: true }, IOC_TYPE.IPV4)
  ...
  it("uses OTX when AbuseIPDB is disabled for IPv4", () => {
    expect(
      resolveEnabledLiveEnrichmentSourceId({ abuseipdb: false, otx: true }, IOC_TYPE.IPV4)
    ).toBe(ENRICHMENT_SOURCE.OTX);
  });

  it("uses OTX for non-IPv4 types when enabled", () => {
    expect(
      resolveEnabledLiveEnrichmentSourceId({ abuseipdb: true, otx: true }, IOC_TYPE.DOMAIN)
    ).toBe(ENRICHMENT_SOURCE.OTX);
  });

  it("returns null when no enabled live source supports the IOC type", () => {
    expect(
      resolveEnabledLiveEnrichmentSourceId({ abuseipdb: false, otx: false }, IOC_TYPE.IPV4)
    ).toBeNull();
    expect(
      resolveEnabledLiveEnrichmentSourceId({ abuseipdb: true, otx: false }, IOC_TYPE.DOMAIN)
    ).toBeNull();
  });
