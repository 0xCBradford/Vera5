/**
 * Phase 21D — offline scoring benchmark corpus (v1) — FROZEN.
 * Do not mutate case expectations for 21D.1 calibration.
...
import { SCORING_BENCHMARK_VERSION_V1 } from "./calibrationTypes";
import {
  BENCHMARK_CATEGORY,
...
/** Frozen Phase 21D corpus (30 cases). */
export function getScoringBenchmarkCorpusV1(): ScoringBenchmarkCase[] {
...
export function getBenchmarkCorpusMetaV1() {
  const cases = getScoringBenchmarkCorpusV1();
  ...
  return {
    version: SCORING_BENCHMARK_VERSION_V1,
    ...
    calibrationCount: cases.filter((c) => c.set === "calibration").length,
  };
}
