...
describe("pageContextStorage", () => {
  ...
  it("skips analyst preset application when the site has a mode override", async () => {
    ...
    expect(applySpy).not.toHaveBeenCalled();
    expect(sessionStore[tabPageContextStorageKey(12)]).toMatchObject({
      pageContextType: PAGE_CONTEXT_TYPE.CTI_PLATFORM,
    });
  });

  it("returns overridden page context through GET_TAB_PAGE_CONTEXT", async () => {
    await saveTabPageContext(3, sampleClassification);
    localStore[storage.STORAGE_KEY_PAGE_CONTEXT_SITE_MODE_OVERRIDES] = {
      "splunk.corp": PAGE_CONTEXT_TYPE.MALWARE_BLOG,
    };
  ...
  it("skips analyst preset application when quiet mode is active", async () => {
    const applySpy = vi.spyOn(storage, "applyAnalystModePreset").mockResolvedValue();
    localStore[storage.STORAGE_KEY_QUIET_MODE] = true;
  ...
  it("skips analyst preset application when domain policy blocks the page origin", async () => {
    const applySpy = vi.spyOn(storage, "applyAnalystModePreset").mockResolvedValue();
    localStore[storage.STORAGE_KEY_DOMAIN_POLICY_MODE] = "allow_by_default";
    localStore[storage.STORAGE_KEY_DOMAIN_DENYLIST] = ["splunk.corp"];
