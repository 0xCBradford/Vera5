...
import { requestLocalBackendEnrichment } from "../lib/localBackendEnrichment";
...
function createLocalBackendNetworkError(
  sourceId: EnrichmentSourceId
): EnrichmentSourceResult {
  return createErrorSourceResult({
    sourceId,
...
async function enrichFromLocalBackend(
  ...
  if (!backendResponse) {
    if (message.sourceId && isEnrichmentSourceId(message.sourceId)) {
      const source = createLocalBackendNetworkError(message.sourceId);
    ...
    const sources = sourceIds.map((sourceId) =>
      createLocalBackendNetworkError(sourceId)
    );
