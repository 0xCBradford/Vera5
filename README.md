...
Browser extension for on-demand IOC detection and triage on `http://` and `https://` pages you browse. Visible-text IOC detection runs by default; optional link attribute scanning is off until you enable it in Settings. Scans, enrichment, scoring, export, investigation sessions, investigation session timelines, same-page **Appeared alongside** co-occurrence, and IOC collections run locally—you bring your own API keys; Vera5 does not operate a shared enrichment cloud. **Quiet mode** (off by default) blocks outbound live vendor enrichment while preserving local detection, cached enrichment display, and attributed pivot links. **Chromium** is the primary install target (`extension/dist/`); an experimental **Firefox** Manifest V3 build ships as `extension/dist-firefox/`. Optional **local AI summary** and an optional localhost FastAPI aggregator both stay on `127.0.0.1`, off by default until you enable them in settings.
...
| **Analyst workflow presets** | **SOC triage**, **CTI research**, and **DFIR investigation** presets apply role-specific defaults: built-in presets enable **AbuseIPDB** and **OTX** only (not URLScan.io, GreyNoise, Shodan, Censys, or other live sources), plus manual-only and notice defaults, optional private-space IPv4 (DFIR), default export template, recommended pivot ordering, and quiet mode (**DFIR investigation** on; SOC and CTI off). |
...
5. Under **Trust & consent**, set pre-query notices, domain policy, internal asset lists if needed, **quiet mode** when you need cache-and-pivot-only triage without outbound vendor API calls, and an analyst workflow preset when useful. Leave **Scan link attributes for IOCs** off unless you need markup-only IOCs on trusted CTI paste hosts—enable only after the first-enable consent dialog.
...
## Example workspace snapshot markdown

Full-workspace markdown bundle excerpts are defined in [docs/export-artifacts.md](docs/export-artifacts.md) as a library contract only. There is no **Export workspace snapshot** or **Import workspace snapshot** control in the shipped extension UI today.
...
| `docs/analyst-workflows.md` | Cache, refresh, keyboard triage, command palette, context-menu enrich, investigation history reopen, source health, **quiet mode**, investigation sessions, **investigation session timeline**, **Appeared alongside** same-page co-occurrence, IOC collections, and quota-aware triage. |
...
|---------------------|---------|
| `storage` | Settings, masked API keys, enrichment cache, analyst notes, investigation sessions (including session timeline events and per-session same-page co-occurrence indexes), investigation history, IOC collections, IOC labels, co-occurrence limit overrides, optional workspace snapshot stored state when import builders run (cache refs and notebook fragment fields; never API keys), per-source last-status snapshots for source operations, toggles (`chrome.storage.local`); per-tab scan summaries for the extension workspace tray (`chrome.storage.session`). |
...
- No maintainer telemetry, crash reporting, or Vera5-operated network endpoints by default.
- Scans and detection run locally. Enrichment sends only the selected indicator value to declared vendor API hosts you enable. **Appeared alongside** uses the local tab scan snapshot only—no page HTML or cross-session indicator correlation leaves the browser. With **Scan link attributes for IOCs** on, allowlisted attribute values are processed locally under the same trust gates—no attribute dumps or page HTML leave the browser. Composite risk labels and reasoning chains are computed locally from vendor summaries; **connector confidence metadata** chips are registry-defined labels only—no cloud metadata feed and no effect on the composite score. Raw JSON panels redact sensitive key fields before display. Production bundles omit sensitive `console` output. **Open live URL** uses your browser only after you confirm; Vera5 does not fetch or proxy that navigation.
