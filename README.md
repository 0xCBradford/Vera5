...
Browser extension for on-demand IOC detection and triage on `http://` and `https://` pages you browse. Visible-text IOC detection runs by default; optional link attribute scanning is off until you enable it in Settings. Scans, enrichment, scoring, export, investigation sessions, investigation session timelines, and IOC collections run locally—you bring your own API keys; Vera5 does not operate a shared enrichment cloud. **Chromium** is the primary install target (`extension/dist/`); an experimental **Firefox** Manifest V3 build ships as `extension/dist-firefox/`. Optional **local AI summary** and an optional localhost FastAPI aggregator both stay on `127.0.0.1`, off by default until you enable them in settings.
...
9. **Optional profile handoff:** Export a **settings pack** from **Settings Backup** on one browser profile and import it on another after reviewing the diff preview—API keys stay local to each profile. For investigation artifacts operators can use today, combine **Session export**, **Timeline appendix export**, and **Collection export**; full-workspace snapshot JSON import/export is library-only until workspace controls ship (see **Workspace snapshot UI** under Limitations). See [docs/analyst-workflows.md — Settings packs](docs/analyst-workflows.md#settings-packs-and-threat-profiles) and [docs/export-artifacts.md](docs/export-artifacts.md).
...
These samples are **full-workspace** markdown bundle excerpts (library contract)—not per-indicator overlay exports, **Session export** tray snapshots, or **Timeline appendix export** alone. There is no workspace button that produces this bundle in the shipped UI today. JSON uses the workspace snapshot document shape with top-level `schemaVersion: 1` and sections for session, tray, enrichment cache references, timeline events, and notebook fragment schema fields.
...
Field contract, import merge/replace behavior, Obsidian zip layout, and wikilink rules: [docs/export-artifacts.md](docs/export-artifacts.md).
...
| `docs/analyst-workflows.md` | Cache, refresh, keyboard triage, command palette, context-menu enrich, investigation history reopen, source health, investigation sessions, **investigation session timeline**, IOC collections, and quota-aware triage. |
...
|---------------------|---------|
| `storage` | Settings, masked API keys, enrichment cache, analyst notes, investigation sessions (including session timeline events), investigation history, IOC collections, IOC labels, optional workspace snapshot stored state when import builders run (cache refs and notebook fragment fields; never API keys), per-source last-status snapshots for source operations, toggles (`chrome.storage.local`); per-tab scan summaries for the extension workspace tray (`chrome.storage.session`). |
...
- API keys, enrichment cache, analyst notes, investigation sessions (including session timeline events), investigation history, and IOC collections stay in local extension storage by default on both Chromium and Firefox; Vera5 does not host enrichment infrastructure.
...
- Settings export omits API keys unless you include them. **Settings packs** never include API keys; import rejects files with secret field names. Workspace snapshot artifact builders apply the same redaction rules when used (library only—no workspace export/import UI today). Pre-migration backup snapshots and migration artifacts stay in local extension storage only—they are not uploaded.
