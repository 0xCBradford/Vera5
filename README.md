...
Browser extension for on-demand IOC detection and triage on `http://` and `https://` pages you browse. Visible-text IOC detection runs by default; optional link attribute scanning is off until you enable it in Settings. Scans, enrichment, scoring, export, investigation sessions (including session timelines and **Investigation replay**), same-page **Appeared alongside** co-occurrence, IOC collections, and operator macros run locally—you bring your own API keys; Vera5 does not operate a shared enrichment cloud. **Local page profile** classifies the active tab from URL and bounded DOM signals only (no page upload or cloud fingerprinting) to tune tray ordering, default export templates, analyst preset defaults, and optional playbook suggestions. **Quiet mode** (off by default) blocks outbound live vendor enrichment while preserving local detection, cached enrichment display, and attributed pivot links. **Chromium** is the primary install target (`extension/dist/`); an experimental **Firefox** Manifest V3 build ships as `extension/dist-firefox/`. Optional **local AI summary** stays on `127.0.0.1`; the extension also includes an off-by-default client bridge for a compatible localhost enrichment service, but this repository does not ship that server.
...
8. Use **Investigation session** (including **Session timeline** filters, **Export timeline appendix**, and **Investigation replay** step-through with **Copy transcript** / **Download transcript**) and **IOC collections** for case tracking and ticket handoff exports.
...
| `docs/analyst-workflows.md` | Cache, refresh, keyboard triage, command palette and context menus, operator macros and trust behavior, investigation history, source health, **quiet mode**, **page profile** defaults, sessions, timeline, **Investigation replay**, **Appeared alongside**, collections, and quota-aware triage. |
...
| `extension/src/lib/` | IOC detection, connector registry and modules, browser compatibility, settings and storage migration, page profiles, operator macro schema/built-ins/storage and key-free packs, workspace snapshot library contracts, sessions, timeline, investigation replay segments and transcript builders, co-occurrence, history, collections, command registry, cache, scoring, export builders/templates, analyst notes, pivots, local AI summary, and UI styles. |
| `extension/prompts/` | Versioned enrichment-summary prompt template for local AI summary requests. |
| `extension/src/popup/` | Shared extension workspace UI (Firefox toolbar popup and side-panel host): scan and selection actions, sessions, timeline export, **Investigation replay**, history, collections, detected-indicators tray with macro actions, and source operations. |
| `extension/src/options/` | Settings page for keys, sources, the optional localhost client bridge/local AI, trust and scanning controls, operator macro editing and packs, cache, and settings backup. |
...
- API keys, enrichment cache, operator macro definitions, analyst notes, investigation sessions (including session timeline events that power **Investigation replay**, and same-page co-occurrence indexes), investigation history, and IOC collections stay in local extension storage by default on both Chromium and Firefox; Vera5 does not host enrichment infrastructure.
...
- No maintainer telemetry, crash reporting, or Vera5-operated network endpoints by default.
- Scans and detection run locally. Enrichment sends only the selected indicator value to declared vendor API hosts you enable. **Page profile** classification uses URL and bounded DOM signals on the active tab only—no page HTML upload, cloud fingerprinting, or cross-tab page content collection. **Appeared alongside** uses the local tab scan snapshot only—no page HTML or cross-session indicator correlation leaves the browser. **Investigation replay** reads the local session timeline only—no screen/video capture, no replay upload, and no live vendor re-enrichment during step-through; transcript copy/download is user-initiated. With **Scan link attributes for IOCs** on, allowlisted attribute values are processed locally under the same trust gates—no attribute dumps or page HTML leave the browser. Composite risk labels and reasoning chains are computed locally from vendor summaries; **connector confidence metadata** chips are registry-defined labels only—no cloud metadata feed and no effect on the composite score. Raw JSON panels redact sensitive key fields before display. Production bundles omit sensitive `console` output. **Open live URL** uses your browser only after you confirm; Vera5 does not fetch or proxy that navigation.
