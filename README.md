...
On **first install**, Vera5 opens **Settings** once for a four-step **Install quick start** wizard (Welcome → optional AbuseIPDB/OTX/URLScan.io keys → enrichment control → trust defaults). Completing the wizard keeps **manual-only enrichment** on and **auto-scan** off unless you change them in the wizard or under **Scanning**.
...
2. Enter your own **AbuseIPDB**, **OTX**, and/or **URLScan.io** API keys (masked in the UI).
...
| **Live enrichment** | Live HTTPS API queries when you enable **AbuseIPDB**, **OTX**, or **URLScan.io** (indicator-type coverage in **What works today**). **GreyNoise**, **VirusTotal**, and the other registry entries do not call vendor APIs—pivot links and settings slots only. |
...
| **Live enrichment** | **AbuseIPDB** (IPv4), **OTX** (IPv4, domain, URL, MD5, SHA1, SHA256, CVE), and **URLScan.io** (domain, URL) when enabled with a saved API key. Enabled live sources are queried in parallel from the service worker; partial success keeps working sources visible while failures stay per source. Only the indicator value is sent to vendors (URLScan.io receives a search query for the domain or URL—not a full page submission). Live HTTPS calls are limited to declared connector API hosts (`api.abuseipdb.com`, `otx.alienvault.com`, `urlscan.io`); a runtime allowlist blocks undeclared outbound fetch before network I/O. Pre-query disclosure (when enabled), domain policy, and internal asset lists gate outbound calls before the service worker fetch runs. |
...
| **Analyst workflow presets** | **SOC triage**, **CTI research**, and **DFIR investigation** presets apply role-specific defaults: AbuseIPDB and OTX enable flags (built-in presets do not auto-enable URLScan.io), manual-only and notice defaults, optional private-space IPv4 (DFIR), default export template, and recommended pivot ordering. |
| **Registered enrichment sources** | Twelve sources appear in settings and the overlay registry (AbuseIPDB, OTX, VirusTotal, URLScan.io, GreyNoise, Shodan, Google Safe Browsing, Pulsedive, MalwareBazaar, Censys, ThreatFox, URLhaus). **AbuseIPDB**, **OTX**, and **URLScan.io** perform live HTTPS enrichment when enabled (URLScan.io: domain and URL only). Other enabled sources may show missing-key, unsupported-type, or not-implemented status rows; pivot links still appear where the registry defines them for the IOC type. Saved API keys for non-live sources stay in local storage and are not sent to those vendors. |
...
2. Open **Vera5 Settings**, enter **AbuseIPDB**, **OTX**, and/or **URLScan.io** keys when you want live enrichment, and enable those sources. Live HTTPS responses come from enabled live connectors (URLScan.io enriches domain and URL indicators only).
...
| `docs/release-notes-v0.1.0.md` | Archived release notes for tag `v0.1.0`; README and [CHANGELOG.md](CHANGELOG.md) reflect current capability. |
...
7. Save API keys and enable AbuseIPDB, OTX, and/or URLScan.io in settings.
...
11. Try a domain or URL with OTX and/or URLScan.io enabled and confirm live per-source rows (for example **URLScan.io · Live** with a result summary), **Recommended next pivots** order, and guidance for that IOC type. Try a hash or CVE with OTX only.
...
| `host_permissions` (`http://*/*`, `https://*/*`) | Content scripts, visible-text reads for detection, and HTTPS enrichment calls (indicator values only) to declared vendor API hosts you enable (AbuseIPDB, OTX, URLScan.io). Pivot recipe links open in the browser like normal navigation. |
...
Pull request CI runs `npm run test:e2e:critical` after building `extension/dist/` (mocked AbuseIPDB/OTX/URLScan.io; no live vendor calls). After `npm run build`:
