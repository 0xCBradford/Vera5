...
Browser extension for on-demand IOC detection and triage on `http://` and `https://` pages you browse. Visible-text IOC detection runs by default; optional link attribute scanning is off until you enable it in Settings. Scans, enrichment, scoring, export, investigation sessions (including session timelines and **Investigation replay**), same-page **Appeared alongside** co-occurrence, cross-session **Appeared across sessions** clusters, IOC collections, and operator macros run locally—you bring your own API keys; Vera5 does not operate a shared enrichment cloud. **Local page profile** classifies the active tab from URL and bounded DOM signals only (no page upload or cloud fingerprinting) to tune tray ordering, default export templates, analyst preset defaults, and optional playbook suggestions. **Quiet mode** (off by default) blocks outbound live vendor enrichment while preserving local detection, cached enrichment display, and attributed pivot links. **Chromium** is the primary install target (`extension/dist/`); an experimental **Firefox** Manifest V3 build ships as `extension/dist-firefox/`. Optional **local AI summary** stays on `127.0.0.1`; the extension also includes an off-by-default client bridge for a compatible localhost enrichment service, but this repository does not ship that server.
...
| **Backend** | No Vera5-operated enrichment relay, shared team workspace, or cloud sync for sessions, collections, macros, replay transcripts, or keys. The extension ships only an off-by-default client bridge for a separately operated compatible service at `127.0.0.1:8765`; this repository does not include the server. If the bridge is enabled but unavailable, Vera5 falls back to in-extension connectors. |
...
| **Session IOC memory** | On the overlay when a session is active: **Label** (Benign, Internal, Suppress false positive, Case important), **Pin** for triage priority, and a compact per-indicator **Session timeline** (first seen, enrich, and export timestamps for that IOC only). Labels and pin state persist locally per IOC value. The workspace **Session timeline** is the full session event log—see **Investigation session timeline**. |
...
| `docs/analyst-workflows.md` | Cache, refresh, keyboard triage, command palette and context menus, operator macros and trust behavior, investigation history, source health, **quiet mode**, **page profile** defaults, sessions, timeline, **Investigation replay**, **Appeared alongside**, **Appeared across sessions**, collections, and quota-aware triage. |
| `docs/soc-validation-fixtures.md` | Splunk-export, Security Onion-style, CTI, malware-blog, sandbox-report, generic, extended-indicator, and link-attribute sample pages for repeatable detection and page-profile checks. |
...
| `docs/security-model.md` | Permissions, host access, CSP, outbound boundaries, hardening checklist, hostile-page DOM guidance, and **Investigation replay** privacy (local-only, read-only playback). |
...
| `examples/` | Sample HTML (SOC dashboard exports, CTI platform, malware-blog and sandbox-report layouts, generic runbook pages, extended-indicator bulletin, benign navigation anchors, attribute-only IOC markup) and IOC strings for manual checks. |
...
| `extension/src/lib/` | IOC detection, connector registry and modules, browser compatibility, settings and storage migration, page profiles, operator macro schema/built-ins/storage and key-free packs, workspace snapshot library contracts, sessions, timeline, investigation replay segments and transcript builders, same-page co-occurrence, cross-session correlation clusters and pack appendix builders, history, collections, command registry, cache, scoring, export builders/templates, analyst notes, pivots, local AI summary, and UI styles. |
| `extension/prompts/` | Versioned enrichment-summary prompt template for local AI summary requests. |
| `extension/src/popup/` | Shared extension workspace UI (Firefox toolbar popup and side-panel host): scan and selection actions, sessions, timeline export, **Investigation replay**, history, collections, detected-indicators tray with **Appeared alongside**, **Appeared across sessions**, and macro actions, and source operations. |
| `extension/src/options/` | Settings page for keys, sources, the optional localhost client bridge/local AI, trust and scanning controls, **Cross-session correlation**, operator macro editing and packs, cache, and settings backup. |
...
- API keys, enrichment cache, operator macro definitions, analyst notes, investigation sessions (including session timeline events that power **Investigation replay**, same-page co-occurrence indexes, and cross-session **Appeared across sessions** clusters), investigation history, and IOC collections stay in local extension storage by default on both Chromium and Firefox; Vera5 does not host enrichment infrastructure.
...
| **Trust gates** | Domain policy, internal asset lists, manual-only enrichment (default), pre-query disclosure, and **quiet mode** limit accidental vendor queries on sensitive pages. |
