...
Browser extension for on-demand IOC detection and triage on `http://` and `https://` pages you browse. Visible-text IOC detection runs by default; optional link attribute scanning is off until you enable it in Settings. Scans, enrichment, scoring, export, investigation sessions (including session timelines), same-page **Appeared alongside** co-occurrence, IOC collections, and operator macros run locally—you bring your own API keys; Vera5 does not operate a shared enrichment cloud. **Local page profile** classifies the active tab from URL and bounded DOM signals only (no page upload or cloud fingerprinting) to tune tray ordering, default export templates, analyst preset defaults, and optional playbook suggestions. **Quiet mode** (off by default) blocks outbound live vendor enrichment while preserving local detection, cached enrichment display, and attributed pivot links. **Chromium** is the primary install target (`extension/dist/`); an experimental **Firefox** Manifest V3 build ships as `extension/dist-firefox/`. Optional **local AI summary** stays on `127.0.0.1`; the extension also includes an off-by-default client bridge for a compatible localhost enrichment service, but this repository does not ship that server.
...
| **Backend** | No Vera5-operated enrichment relay, shared team workspace, or cloud sync for sessions, collections, macros, or keys. The extension ships only an off-by-default client bridge for a separately operated compatible service at `127.0.0.1:8765`; this repository does not include the server. If the bridge is enabled but unavailable, Vera5 falls back to in-extension connectors. |
...
| **Investigation session timeline** | Chronological **Session timeline** under **Investigation session** in the extension workspace. Records **First seen**, **Enriched**, **Exported**, **Tagged**, **Macro run** (for example context-menu **Enrich selection with Vera5**), and **Seen again** as you scan, enrich, export, assign labels, or rescan the page. Filter by indicator, event type, or UTC time range (**From** / **To**). Click a row when the indicator is on the current tab to scroll to the page highlight and open the workspace detail pane; rescan when the indicator is missing from the tray. Stores up to **100** events per session with deduplication of near-duplicate events within a short window. Local storage only—no cloud sync or SIEM export. |
...
| `docs/analyst-workflows.md` | Cache, refresh, keyboard triage, command palette and context menus, operator macros and trust behavior, investigation history, source health, **quiet mode**, **page profile** defaults, sessions and timeline, **Appeared alongside**, collections, and quota-aware triage. |
...
| `extension/src/lib/` | IOC detection, connector registry and modules, browser compatibility, settings and storage migration, page profiles, operator macro schema/built-ins/storage and key-free packs, workspace snapshot library contracts, sessions and timeline, co-occurrence, history, collections, command registry, cache, scoring, export builders/templates, analyst notes, pivots, local AI summary, and UI styles. |
| `extension/prompts/` | Versioned enrichment-summary prompt template for local AI summary requests. |
| `extension/src/popup/` | Shared extension workspace UI (Firefox toolbar popup and side-panel host): scan and selection actions, sessions and timeline export, history, collections, detected-indicators tray with macro actions, and source operations. |
...
- API keys, enrichment cache, operator macro definitions, analyst notes, investigation sessions (including session timeline events and same-page co-occurrence indexes), investigation history, and IOC collections stay in local extension storage by default on both Chromium and Firefox; Vera5 does not host enrichment infrastructure.
...
- No maintainer telemetry, crash reporting, or Vera5-operated network endpoints by default.
- Scans and detection run locally. Enrichment sends only the selected indicator value to declared vendor API hosts you enable. **Page profile** classification uses URL and bounded DOM signals on the active tab only—no page HTML upload, cloud fingerprinting, or cross-tab page content collection. **Appeared alongside** uses the local tab scan snapshot only—no page HTML or cross-session indicator correlation leaves the browser. With **Scan link attributes for IOCs** on, allowlisted attribute values are processed locally under the same trust gates—no attribute dumps or page HTML leave the browser. Composite risk labels and reasoning chains are computed locally from vendor summaries; **connector confidence metadata** chips are registry-defined labels only—no cloud metadata feed and no effect on the composite score. Raw JSON panels redact sensitive key fields before display. Production bundles omit sensitive `console` output. **Open live URL** uses your browser only after you confirm; Vera5 does not fetch or proxy that navigation.
