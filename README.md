...
| **Settings packs** | Export a **settings pack** (`vera5-settings-pack.json`) from **Settings Backup** to share connector toggles, cache TTL, domain policy, and analyst mode—never API keys. **Import settings pack** shows a field-by-field diff preview before you apply; stored API keys on the profile stay unchanged. Separate from full **Settings Backup** export/import (optional API keys). The pack importer rejects threat-profile-shaped JSON. See [docs/analyst-workflows.md — Settings packs](docs/analyst-workflows.md#settings-packs-and-threat-profiles) and [docs/security-model.md](docs/security-model.md). |
...
9. **Optional profile handoff:** Export a **settings pack** from **Settings Backup** on one browser profile and import it on another after reviewing the diff preview—API keys stay local to each profile. For investigation artifacts operators can use today, combine **Session export**, **Timeline appendix export**, and **Collection export**; full-workspace snapshot JSON import/export has no workspace UI controls today (see **Workspace snapshot UI** under Limitations). See [docs/analyst-workflows.md — Settings packs](docs/analyst-workflows.md#settings-packs-and-threat-profiles) and [docs/export-artifacts.md](docs/export-artifacts.md).
